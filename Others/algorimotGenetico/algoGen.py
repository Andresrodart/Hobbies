...
class AlgoGen:
	...
	def algoritmo(self, indi=None, pobls=None, bitPres=None):		#Podemos recalcular el mejor valor con nuevos individuos, o mas poblaciones o con otro bit de presicion
		...
		for i in range(self.rules["poblaciones"]):
			resultFO = [None]*(len(self.individuos) + 1)			#Aqui guardaremos lso reultados de cada individuo
			resPorFO = [[None, 0] for i in range(len(self.individuos))]	#Aquí gurdaremos la Z% y Z% acumulado
			indiFrts = {}											#Aquí guardaremos los id de los invividuos que salgan despues de elegir el numero aleaotrio
			resultFO[-1] = 0										#La última posición guarda la suma de todas las evaluaciones en F.O.
			for j in range(len(self.individuos)):
				varHelp = self.individuos[j][1]						#Auxiliar para evaluar en la F.O.
				resultFO[j] = eval(self.rules['FO'], None,varHelp)		#Evaluamos al individuo en F.O.
				resultFO[-1] += resultFO[j]							#Z acumulado
			
			resPorFO[0][0] = resultFO[0]/resultFO[-1]				#Z% = resZ_j / Zacumulado
			resPorFO[0][1] = resPorFO[0][0]							#Z% acumulado	
			for j in range(1, len(self.individuos)):
				resPorFO[j][0] = resultFO[j]/resultFO[-1]			#Z% = resZ_j / Zacumulado
				resPorFO[j][1] = resPorFO[j - 1][1]	+ resPorFO[j][0]#Z% acumulado
			for j in range(len(self.individuos)):					#Buscamos los individuos mas fuertes
				ranNum = random.random()
				for k in range(len(resPorFO)):						#Buscamos en cual de los rangos entra la variable
					if ranNum <= resPorFO[k][1]:
...
if __name__ == '__main__':
	...
	window.show()
	app.exec_()
	#print('\n'.join(str(elem) for elem in indiduals))